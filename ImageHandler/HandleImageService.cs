...
namespace Jtext103.ImageHandler
{
    public class HandleImageService
    {
        ...
        public static void ThumbnailImage(Stream fileStream, string path, int width, int height)
        {
            Image origin = Image.FromStream(fileStream);
            //只有原图长宽都大于所需图片长宽时才进行缩略
            if (origin.Width > width && origin.Height > height)
            {
                Image thumbnail = origin.GetThumbnailImage(width, height, null, IntPtr.Zero);
